...
export class UserService {
    ...
    async emailSignUp(files: File[], body: RegistUserReqDto): Promise<any> {
        try {
            ...
            if (user) {
                const expire: Date = new Date(user.createdAt);
                ...
                if (body.recommendCode !== '') {
                    ...
                    //추천 받은 사람
                    const recommendUser: User = await this.userRepository.findByAny(
                        'userCode',
                        body.recommendCode,
                    );
                    ...
                    await this.pointRepository.create({
                        ...
                        description:
                            recommendUser.currency === Currency.USD
                                ? 'Webudding Referral code'
                                : '위버딩 추천인 코드',
